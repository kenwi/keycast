...
Column {
  ...
  function toSpin(n) {
    var v = Math.round(Number(n) * factor)
    if (!isFinite(v)) return Math.round(root.from * factor)
    return v
  }
  ...
  QQC.SpinBox {
    ...
    from: root.toSpin(root.from)
    to: root.toSpin(root.to)
    stepSize: Math.max(1, root.toSpin(root.stepSize))
    value: root.toSpin(root.value)
    editable: true
    font.family: root.fontFamily
    ...
    valueFromText: function(text, locale) {
      ...
      if (!isFinite(n)) n = Number(String(text).replace(",", "."))
      if (!isFinite(n)) return spin.value
      return root.toSpin(n)
    }

    onValueModified: root.modified(value / root.factor)
